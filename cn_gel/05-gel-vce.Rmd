...
where $y$ is a vector of length $N$ of observations, $b$ is a vector of length $p$ of fixed effects, $u$ is a vector of length $q$ of random breeding values and $e$ is a vector of length $N$ of random errors. The matrices $X$ and $Z$ are design matrices linking the corresponding effects to the observations. As part of the model definition, the variances of the random effects are defined as 
...
In \@ref(eq:gelvcevaravare) $\sigma_u^2$ and $\sigma_e^2$ are the variance components that must be estimated from the data. The material presented in this chapter is based on `r met$add("Essl1987")` and `r met$add("Searle1992")` and it shows different methods how variance components for different models can be estimated. 
...
One first method that we want to introduce is based on the very well-known statistical technique called __analysis of variance__ (ANOVA). ANOVA is shown in the next subsection for a simple application of estimating the repeatability. Later this can be generalized to the estimation of the variability due to genetic components. 
