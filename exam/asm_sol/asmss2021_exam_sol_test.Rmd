...
\begin{enumerate}
\item[b)] Use a single-step marker-effect model to predict breeding values using the data shown above. Please specify the model type and all the components of the marker-effect model and describe how genomic breeding values are predicted.
...
### Master Solution
In a single-step marker effect model a mixed linear effect model is used to predict marker effects. Marker effects are treated as random effects. Genomic breeding values are predicted by summing over the appropriate marker effects.
...
\begin{enumerate}
\item[c)] Use a single-step Genomic BLUP (GBLUP) model to predict genomic breeding values. Please specify the type of model used and also list all the model components of the GBLUP model used. 
...
### Master Solution 
The GBLUP model is a mixed linear effect model where genomic breeding values are treated as random effects. 
