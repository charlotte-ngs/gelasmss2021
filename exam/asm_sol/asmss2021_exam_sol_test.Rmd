...
if (file.exists(s_data_p04_local_path)){
  tbl_data <- readr::read_csv2(file = s_data_p04_local_path)
} else {
  # pedigree
  n_nr_ani <- 5
  n_nr_founder <- 3
  n_nr_non_founder <- n_nr_ani - n_nr_founder
  tbl_ped <- tibble::tibble(ID = c((n_nr_ani - n_nr_founder + 2):n_nr_ani),
                            Sire = c(1, 1),
                            Dam  = c(2, 3))
  ...
  mat_sim_geno <- genotype[,c(1,129,2)];mat_sim_geno
  
  # determine the number of SNP
  n_nr_snp <- ncol(mat_sim_geno)
  
  # simulate observations
  vec_snp_eff <- c(13.3, 2.9, 1.7)
  vec_sex <- c("M" = 7.1, "F" = 3.3);vec_sex
  sd_res <- 1.9
  n_intercept <- -5.3
  ...
  for (idx in 1:n_nr_snp){
    tbl_data <- dplyr::bind_cols(tbl_data, tibble::tibble(SNP = mat_sim_geno[,idx]-1))
    names(tbl_data)[ncol(tbl_data)] <- paste0("SNP", idx)
  }
  
  # write data to file
  readr::write_csv2(tbl_data, file = s_data_p04_local_path)
}
