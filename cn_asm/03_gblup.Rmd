...
```{r met-gblup-reset, include=FALSE}
...
<!-- 
  -- TODO: Move the model selection part to the FLEM chapter
  -- TODO: Use Fernando2014 as a template for the structure of the chapter
  -- TODO: Use Chapter 13 of Gondro2013 for the examples section
-->

The content of this chapter is based on `r met$add("Fernando2014")` and chapter 13 of `r met$add("Gondro2013")`. In this chapter the different models that extend the fixed linear model to predict genomic breeding values are described. The described methods are illustrated with an example analysis that demonstrates the advantage of a genomic evaluation compared to a traditional analysis based on only phenotypes and pedigree data. The topic of alternative parameter estimation approaches is deferred to chapters \@ref(asm-lasso) and \@ref(asm-bayes).
...
```{r selectsnploci, echo=FALSE, hook_convert_odg=TRUE, fig_path="odg", fig.cap="Finding SNP Loci Important For The Expression Of A Quantitative Trait", out.width='11cm'}
#rmdhelp::use_odg_graphic(ps_path = "odg/selectsnploci.odg")
...
3. Continue with step 2 until all predictor variables are included. This results in a series of models $\mathcal{M}_0 \subseteq \mathcal{M}_1 \subseteq \mathcal{M}_2 \subseteq ...$.
4. From the series of models select the one that optimizes a previously defined selection criterion.

As model quality criterion, different quantities can be used. Possible quantities are the Mallow $C_p$ criterion, the Akaike Information Criterion (AIC) or Bayes Information Criterion (BIC). 
...
3. Continue with step 2 until all predictor variables are dropped. This results in a series of models $\mathcal{M}_0 \subseteq \mathcal{M}_1 \subseteq \mathcal{M}_2 \subseteq ...$.
...
In real-world genomic data analyses, the number of predictors ($p$) can be as high as $1.5 * 10^5$. Therefore the backward elimination approach as described in \@ref(asm-gblup-backward) cannot be used because of the problem of $n <<p$ . Also the forward selection approach does not yield a stable procedure for finding the subset of relevant SNPs. This has several reasons which are shortly described in the following subsections. 
